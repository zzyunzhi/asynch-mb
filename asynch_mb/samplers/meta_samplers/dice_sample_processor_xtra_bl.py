from asynch_mb.utils import utils
...
class DiceSampleProcessor(SampleProcessor):
    ...
    def _compute_samples_data(self, paths):
        ...
        # 4) stack path data
        mask, observations, actions, rewards, baselines, adjusted_rewards, env_infos, agent_infos = self._pad_and_stack_paths(paths)

        # 5) if desired normalize / shift adjusted_rewards
        if self.normalize_adv:
            # compute reward statistics
            mean, std = np.mean(adjusted_rewards), np.std(adjusted_rewards)

            # normalize adj_rewards, rewards, baselines
            adjusted_rewards = (adjusted_rewards - mean) / (std + 1e-8)
