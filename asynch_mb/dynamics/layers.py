...
from asynch_mb.utils.utils import remove_scope_from_name
from asynch_mb.dynamics.utils import *
...
class CNN(Layer):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.input_var, self.cnn_output_var = create_cnn(name='cnn',
                                                             hidden_nonlinearity=self.hidden_nonlinearity,
            ...
            _, self.output_var = create_mlp(name='mlp',
                                            output_dim=self.output_dim,
                                            hidden_sizes=self.hidden_sizes,
                                            hidden_nonlinearity=self.hidden_nonlinearity,
                                            output_nonlinearity=self.output_nonlinearity,
...
class DNN(Layer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.hidden_nonlinearity = kwargs.get('hidden_nonlinearity')
        self.output_nonlinearity = kwargs.get('output_nonlinearity')
        self.input_decnn_dim = kwargs.get('input_decnn_dim')
        self.hidden_dim = kwargs.get('hidden_dim')
        self.n_channels = self.output_dim[2]
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.input_var, self.mlp_output_var = create_mlp(name='mlp',
                                                             output_dim=self.n_channels * self.hidden_dim**2,
                                                             hidden_sizes=self.hidden_sizes,
                                                             hidden_nonlinearity=self.hidden_nonlinearity,
                                                             output_nonlinearity=self.hidden_nonlinearity,  # yes this is correct
                                                             input_dim=(None, self.input_dim,),
                                                             input_var=self.input_var,
                                                             )

            _, self.output_var = create_dnn(name='dnn',
                                            output_dim=self.output_dim,
                                            hidden_nonlinearity=self.hidden_nonlinearity,
                                            output_nonlinearity=self.output_nonlinearity,
                                            kernel_sizes=self.kernel_sizes,
